...
class SubscriptionCubit extends Cubit<SubscriptionState> {
  ...
  SubscriptionCubit(this._repository) : super(const SubscriptionState.initial());

  Future<void> getSubscriptions({bool activeOnly = true}) async {
    emit(state.copyWith(isLoading: true, error: null));

    try {
      final subscriptions = await _repository.getSubscriptions(activeOnly: activeOnly);
      emit(state.copyWith(isLoading: false, subscriptions: subscriptions, error: null));
    } catch (e) {
    ...
    try {
      ...
      emit(
        state.copyWith(
          ...
          unseenProfilesPage: result.unseenProfiles?.pagination.page ?? state.unseenProfilesPage,
          unseenProfilesLimit: result.unseenProfiles?.pagination.limit ?? state.unseenProfilesLimit,
          unseenProfilesTotal: result.unseenProfiles?.pagination.total ?? 0,
          unseenProfilesTotalPages: result.unseenProfiles?.pagination.totalPages ?? 0,
          seenProfilesPage: result.seenProfiles?.pagination.page ?? state.seenProfilesPage,
          seenProfilesLimit: result.seenProfiles?.pagination.limit ?? state.seenProfilesLimit,
          seenProfilesTotal: result.seenProfiles?.pagination.total ?? 0,
          seenProfilesTotalPages: result.seenProfiles?.pagination.totalPages ?? 0,
          error: null,
        ),
      );
    } catch (e) {
  ...
  Future<void> unlock({required ContentType contentType, required String targetId}) async {
    ...
    try {
      ...
      if (result.data != null && result.data!.contentData != null) {
        ...
        if (contentType == ContentType.profileContact && result.data!.contentData is ProfileModel) {
        ...
        final isInUnseen = state.unseenShipmentRecords.any((record) => record.id == targetId);

        if (isInUnseen) {
          // Remove from unseen
          final updatedUnseenRecords = state.unseenShipmentRecords.where((record) => record.id != targetId).toList();
        ...
        if (contentType == ContentType.profileContact) {
          ...
          try {
            profileInUnseen = state.unseenProfiles.firstWhere((profile) => profile.id == targetId);
          } catch (e) {
          ...
          final updatedExploration = state.marketExploration?.copyWith(
            competitiveAnalysis: contentType == ContentType.competitiveAnalysis
                ? state.marketExploration?.competitiveAnalysis?.copyWith(isSeen: true)
  ...
  Future<void> getShipmentRecords({required String profileId, int? seenPage, int? seenLimit, int? unseenPage, int? unseenLimit}) async {
    emit(state.copyWith(isLoading: true, error: null));
    ...
    try {
      ...
      emit(
        state.copyWith(
          ...
          seenShipmentRecordsPage: result.seenShipmentRecords?.pagination.page ?? state.seenShipmentRecordsPage,
          seenShipmentRecordsLimit: result.seenShipmentRecords?.pagination.limit ?? state.seenShipmentRecordsLimit,
          seenShipmentRecordsTotal: result.seenShipmentRecords?.pagination.total ?? 0,
          seenShipmentRecordsTotalPages: result.seenShipmentRecords?.pagination.totalPages ?? 0,
          unseenShipmentRecordsPage: result.unseenShipmentRecords?.pagination.page ?? state.unseenShipmentRecordsPage,
          unseenShipmentRecordsLimit: result.unseenShipmentRecords?.pagination.limit ?? state.unseenShipmentRecordsLimit,
          unseenShipmentRecordsTotal: result.unseenShipmentRecords?.pagination.total ?? 0,
          unseenShipmentRecordsTotalPages: result.unseenShipmentRecords?.pagination.totalPages ?? 0,
